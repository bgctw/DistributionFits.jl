function test_univariate_fits(d, D=typeof(d))
    @testset "fit moments" begin
        if !occursin("fit(::Type{D}", string(first(methods(fit, (Type{typeof(d)}, AbstractMoments)))))
    ...
    @testset "quantile is of eltype" begin
        ...
        if d isa Union{LogNormal, LogitNormal, Exponential, Laplace, Weibull} && partype(d) != eltype(d)
    ...
    @testset "fit to quantilepoint and mean" begin
        if !occursin("fit_mean_quantile(::Type{D}", string(first(
            methods(fit_mean_quantile, (Type{typeof(d)}, partype(d), QuantilePoint)))))
            m = log(mean(d))
            qp = @qp_u(quantile(d,0.95))
            logger = d isa Exponential ? MinLevelLogger(current_logger(), Logging.Error) : current_logger()
