...
@testset "Logitnormal seek mode" begin
    #plot(g); vline!([mode(g)])
    # median
    g = LogitNormal(); test_mode_larger_than_neighbors(g)
    g = LogitNormal(0,1.4); test_mode_larger_than_neighbors(g) 
    # larger one of two modes, infer the lower one
    g = LogitNormal(0,1.6); test_mode_larger_than_neighbors(g)
    # two modes, larger is on the right
    g = LogitNormal(0.1,1.6); test_mode_larger_than_neighbors(g)
    # two modes, larger is on the left
    g = LogitNormal(-0.1,1.6); test_mode_larger_than_neighbors(g)
    #
    # test with Float32
    g = LogitNormal(0f0,1.4f0); 
