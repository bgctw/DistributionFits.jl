...
function fit_mode_quantile(::Type{LogitNormal}, mode::T, qp::QuantilePoint) where T <: Real
...
function matchModeUpper(
    mode::T, upper  # partype only defined by mode - TODO define eltype by upper
    , ::Val{nTry}
    ; perc::Real = 0.99
) where {nTry, T<:Real}
...
```
"""
function fit_mode_flat(::Type{LogitNormal}, mode::T, ::Val{nTry} = Val(40); peakedness = 1) where {T<:Real,nTry}
    fit_mode_flat(LogitNormal{T}, mode, Val(nTry); peakedness)
end
function fit_mode_flat(::Type{LogitNormal{T}}, mode, ::Val{nTry} = Val(40); peakedness = 1) where {T<:Real,nTry}
